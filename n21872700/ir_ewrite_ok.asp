...
<%
	...
	elseif cd="edit" then
		...
		sql="insert nc_ir (i_id,i_title,i_regdate,i_file_idx,i_file_name,i_content,i_ip,lan_type) value (?,?,?,?,?,?,?,1)"
...
<script language="javascript">
	<% if cd="delete" then %>
		alert("삭제되었습니다.");
		parent.location.href="<%= gb %>_elist.asp<%= p_cmd %><%= g_cmd %><%= s_cmd %>";
	<% elseif cd="edit" then %>
		alert("수정되었습니다.");
		parent.location.href="<%= gb %>_eview.asp<%= p_cmd %><%= g_cmd %><%= s_cmd %>&idx=<%= idx %>";
	<% else %>
		alert("등록되었습니다.");
		parent.location.href="<%= gb %>_elist.asp?page=1<%= g_cmd %>";
	<% end if %>
</script>
