...
'		sql="update nc_product set p_order=p_order-1 where p_display>0 and p_order > (select p_order from nc_product where idx = ? ) "
	...
	elseif cd="edit" then
		...
		sql="insert nc_product (p_id,p_div2,p_title,p_file_idx,p_file_name,p_content,p_comment,p_link,p_certify,p_order,p_ip,p_div) values (?,?,?,?,?,?,?,?,?,?,?,(select idx from nc_category where c_category ='"&p_div&"' ))"
