...
<%
	sql="select count(*) from nc_ir where i_display>0 and lan_type = 0" & s_sql
...
<%
	...
	<td width="365" height="27" class="space">
		<a href="<%= gb %>_view.asp<%= p_cmd %><%= g_cmd %><%= s_cmd %>&idx=<%= idx %>"><%= i_title %><%= new_tag %></a>
	</td>
