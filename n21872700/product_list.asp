...
<%
	sql="select count(*) from nc_product where p_display>0" & d_sql & s_sql
	...
	if div<>"" then
		cmd.parameters.append cmd.createparameter("@div",adWChar,adParamInput,255,div)
	end if
...
<%
						sql="select"
						sql=sql & "		idx, p_div, p_title, p_file_idx, p_file_name, p_certify, p_div2"
