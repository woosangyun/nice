...
<%
	if s_str<>"" then
		select case s_part
			case "multi"		: s_sql=" and (a_title like ? or a_content like ? )"
			case "title"			: s_sql=" and a_title like ? "
			case "content"	: s_sql=" and a_content like ? "
		end select
	end if
%>
...
<%
	sql="select count(*) from nc_agreement where a_display>0" & s_sql
...
<%
	...
	<td width="540" height="27" class="space">
		<a href="<%= gb %>_view.asp<%= p_cmd %><%= g_cmd %><%= s_cmd %>&idx=<%= idx %>"><%= a_title %></a>
	</td>
	<td width="55" height="27"> 
