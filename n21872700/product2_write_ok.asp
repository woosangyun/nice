...
<%
	...
	if cd="delete" then
'		sql="update nc_product2 set p_order=p_order-1 where p_display>0 and p_order > (select p_order from nc_product2 where idx = ? ) "
		sql="update nc_product2 set p_order=p_order-1 where p_display>0 and p_order > (select * from (select p_order from nc_product2 a where idx = ? ) as a)"
		...
		sql="update nc_product2 set p_display=0 where idx= ? "
	...
	elseif cd="edit" then
		...
		if old_order-p_order < 0 then
			sql="update nc_product2 set p_order=p_order-1 where p_display>0 and p_order > ? and p_order <= ? "
		...
		sql="update nc_product2 set p_order=p_order+1 where p_display>0 and p_order >= ? "
